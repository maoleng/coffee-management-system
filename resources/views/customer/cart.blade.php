                    ...
                    <div class="col-xl-12">
                        ...
                        @if (authed() !== null)
                            <div class="cart-bottom-wrapper d-flex align-items-center">
                                <form action="{{ route('add_promotion') }}" method="post" class="coupon d-flex align-items-center">
                                    @csrf
                                    <input id="coupon_code" class="input-text" name="code" value="" placeholder="Coupon code" type="text">
                                    @if (session()->get('error') !== null)
                                        {{ session()->get('error') }}
                                    @endif
                                    <button class="site-btn site-btn site-btn__bghide" name="apply_coupon" type="submit">Apply promotion</button>
                                </form>
                            </div>
                        @endif
                    </div>
